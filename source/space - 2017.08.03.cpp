...
double Particle::getVelScalar(){

	return pow((pow(velocity[0], 2.0) + pow(velocity[1], 2.0) + pow(velocity[2], 2.0)), 0.5);
}

double Particle::getAccScalar(){

	return pow((pow(acceleration[0], 2.0) + pow(acceleration[1], 2.0) + pow(acceleration[2], 2.0)), 0.5);
}
...
/*
void calculateAcceleration(Particle entity1, Particle entity2){
...
double calculateDistance(Particle entity1, Particle entity2){
	// Calculates the distance between particles

	double radial = pow((pow(entity2.getPosIndex(0) - entity1.getPosIndex(0), 2.0) + 
					pow(entity2.getPosIndex(1) - entity1.getPosIndex(1), 2.0) + 
					pow(entity2.getPosIndex(2) - entity1.getPosIndex(2), 2.0)), 0.5);
...
void adjustAcceleration(Particle entity1, Particle entity2){
	/*
		This function calculates the acceleration of entity2 acting on entity1 and adjusts
		the acceleration of entity1.
	*/

	double temp[3];
	double* clone = new double[3];
	double gravitation = UGC*entity2.getMass()/pow(calculateDistance(entity1, entity2), 3.0);

	for (int i = 0; i < 3; i++){
		clone[i] = (gravitation*(entity2.getPosIndex(i) - entity1.getPosIndex(i)));
		cout << clone[i] << endl;
	}

	//cout << entity1.getAccIndex(0) << endl;

	/*
		For some reason here, the <>.setAcceleration() function is not modifying the members
		of the Particle object to the new values it is being assigned.
	*/

	temp[0] = entity1.getAccIndex(0)+(-1);//clone[0];
	temp[1] = entity1.getAccIndex(1)+(2);//clone[1];
	temp[2] = entity1.getAccIndex(2)+(-3);//clone[2];
	entity1.setAcceleration(temp[0], temp[1], temp[2]);
...
int main(void){
	...
	entity[1].setPosition(3.84399*pow(10.0, 8.0), 1, -1);
	entity[1].setVelocity(0, 1.022*pow(10.0, 3.0), 0);
	entity[1].setAcceleration(0,0,0);

	//cout << entity[0].getAccScalar() << endl;
	//cout << calculateDistance(entity[0], entity[1]) << endl;
	adjustAcceleration(entity[0], entity[1]);

	cout << entity[0].getAccIndex(0) << "\t" << entity[0].getAccIndex(1) << "\t" << entity[0].getAccIndex(2) << endl;

	entity[0].setAcceleration(-1, 2, -3);
